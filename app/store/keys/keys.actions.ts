...
export function setPassword({ password, history }: { password: string; history: History }) {
  return async (dispatch: Dispatch, getState: () => RootState) => {
    ...
    dispatch(addData({ password, salt, derivedEncryptionKey }));
...
export const attemptWalletDecryptSuccess = createAction('keys/attempt-wallet-decrypt-success');
...
export function decryptWallet({ password, history }: { password: string; history: History }) {
  return async (dispatch: Dispatch, getState: () => RootState) => {
    ...
    if (mnemonic) {
      dispatch(addData({ salt, mnemonic }));
      history.push(routes.HOME);
      dispatch(attemptWalletDecryptSuccess());
    }
  };
}
